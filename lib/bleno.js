...
Bleno.prototype.startAdvertising = function(name, serviceUuids, mac, ip, serviceStatus, callback) {
  if (this.state !== 'poweredOn') {
    ...
    var rawMac = [];
    if (mac && mac.length) {
      rawMac = UuidUtil.removeColons(mac);
    }

    var rawIP = [];
    if (ip && ip.length) {
      rawIP = ip.split(['.']).map(function(item) {return parseInt(item, 10)})
    }

    this._bindings.startAdvertising(name, undashedServiceUuids, rawMac, rawIP, serviceStatus);
  }
};
