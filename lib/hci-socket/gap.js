...
Gap.prototype.startAdvertising = function(name, serviceUuids, mac, ip, serviceStatus) {
  debug('startAdvertising: name = ' + name + ', mac= ' + mac + ', serviceUuids = ' + JSON.stringify(serviceUuids, null, 2));
  ...
  if (mac && mac.length === 12) {
    // Length = 0x09 = 1 byte (0x16 - service type) + 2 bytes service UUID (0x2A25 - serial number) + 6 bytes MAC
    advertisementData.writeUInt8(0x09, advertisementDataOffset);
    advertisementDataOffset++;

    advertisementData.writeUInt8(0x16, advertisementDataOffset);
    advertisementDataOffset++;

    advertisementData.writeUInt8(0x2A, advertisementDataOffset);
    advertisementDataOffset++;

    advertisementData.writeUInt8(0x25, advertisementDataOffset);
    advertisementDataOffset++;

    for (var i = 0; i < mac.length; i += 2) {
      advertisementData.writeUInt8(parseInt(mac.substr(i,2), 16), advertisementDataOffset);
      advertisementDataOffset++;
    }
  }
  ...
  if (serviceStatus && serviceStatus.length) {
    scanDataLength += 4 + serviceStatus.length;
  }
  if (scanDataLength > 31) {
    scanDataLength = 31;
  }
  ...
  // IP
  if (ip && ip.length === 4) {
    // Length = 0x07 = 1 byte (0x16 - service type) + 2 bytes service UUID (0x1820 - IP Support Service) + 4 bytes IP
    scanData.writeUInt8(0x07, scanDataOffset);
    scanDataOffset++;

    scanData.writeUInt8(0x16, scanDataOffset);
    scanDataOffset++;

    scanData.writeUInt8(0x18, scanDataOffset);
    scanDataOffset++;

    scanData.writeUInt8(0x20, scanDataOffset);
    scanDataOffset++;

    for (i in ip) {
      scanData.writeUInt8(ip[i], scanDataOffset);
      scanDataOffset++;
    }
  }
  ...
  // Services Status
  if (serviceStatus && serviceStatus.length) {
    // Length = serviceStatus.length + 1 byte (0x16 - service type) + 2 bytes service UUID (0x181C User Data)
    scanData.writeUInt8(3 + serviceStatus.length, scanDataOffset);
    scanDataOffset++;
    ...
    scanData.writeUInt8(0x1C, scanDataOffset);
    scanDataOffset++;

    for(i in serviceStatus) {
      scanData.writeUInt8(serviceStatus[i], scanDataOffset);
      scanDataOffset++;
    }
  }

  // Name - requires at least 3 bytes left in Scan Response
  if (name && name.length && scanDataOffset < 29) {
    var advertisedNameLength = (name.length > 29 - scanDataOffset) ? 29 - scanDataOffset : name.length;
    var nameBuffer = new Buffer(name);
    // Trim the name if it's too long
    nameBuffer = nameBuffer.slice(0, advertisedNameLength);

    scanData.writeUInt8(1 + advertisedNameLength, scanDataOffset);
    scanDataOffset++;
    ...
    scanDataOffset += advertisedNameLength;
  }
...
Gap.prototype.restartAdvertising = function() {
  if ((this._advertiseState !== 'started') && (this._advertiseState !== 'restarting')) {
