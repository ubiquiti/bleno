...
Gap.prototype.startAdvertising = function(name, serviceUuids, serviceData) {
  ...
  if (advertisementDataLength > 31) {
    // Trim advertisement data if exceeds 31B limit
    advertisementDataLength = 31;
  }
  ...
  if (name && name.length && advertisementDataOffset < 29) {
    var advertisedNameLength = (name.length > 29 - advertisementDataOffset) ? 29 - advertisementDataOffset : name.length;
  ...
  for (i in serviceData) {
    if (serviceData[i].length === 2 && serviceData[i][0].length === 2 && serviceData[i][1].length) {
      scanDataLength += 4 + serviceData[i][1].length
    }
  }
  ...
  for (i in serviceData) {
    if (serviceData[i].length === 2 && serviceData[i][0].length === 2 &&
        serviceData[i][1].length && serviceData[i][1].length < (27 - scanDataOffset)) {
      // Data length = serviceStatus.length + 1 byte (0x16 - service type) + 2 bytes service UUID
      ...
      for (j in serviceData[i][0]) {
        scanData.writeUInt8(serviceData[i][0][j], scanDataOffset);
        scanDataOffset++;
      }
      // Write Service Data
      for (j in serviceData[i][1]) {
        scanData.writeUInt8(serviceData[i][1][j], scanDataOffset);
        scanDataOffset++;
      }
    }
  }
