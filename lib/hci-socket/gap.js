...
Gap.prototype.startAdvertising = function(name, serviceUuids, serviceData) {
  ...
  for (i in serviceData) {
    if (serviceData[i].uuid && serviceData[i].uuid.length === 2 &&
        serviceData[i].data && serviceData[i].data.length) {
      scanDataLength += 4 + serviceData[i].data.length;
    }
  }
  ...
  for (i in serviceData) {
    if (serviceData[i].uuid && serviceData[i].uuid.length === 2 &&
        serviceData[i].data.length && serviceData[i].data.length + scanDataOffset < 28) {
      // Data length = serviceData.data.length + 1 byte (0x16 - service type) + 2 bytes service UUID
      scanData.writeUInt8(3 + serviceData[i].data.length, scanDataOffset);
      scanDataOffset++;
      // Data type 0x16 - Service Data
      scanData.writeUInt8(0x16, scanDataOffset);
      scanDataOffset++;
      // Write 2B Service Data UUID
      for (j in serviceData[i].uuid) {
        scanData.writeUInt8(serviceData[i].uuid[j], scanDataOffset);
        scanDataOffset++;
      }
      // Write Service Data
      for (j in serviceData[i].data) {
        scanData.writeUInt8(serviceData[i].data[j], scanDataOffset);
        scanDataOffset++;
      }
    }
  }
